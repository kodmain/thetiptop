...
func TestStores(t *testing.T) {
	...
	for _, encoding := range encodingTypes {
		...
		t.Run("Store/"+encodingName, func(t *testing.T) {
			...
			t.Run("StoreByID/"+encodingName, func(t *testing.T) {
				for _, store := range stores {
					content, status, err := request("GET", DOMAIN+"/store/"+store.ID, authorization, encoding, nil)
					assert.Nil(t, err)
					assert.Equal(t, http.StatusOK, status)
					var store entities.Store
					assert.Nil(t, json.Unmarshal(content, &store), "Response should be valid JSON")
					assert.NotNil(t, store, "Response should not be nil")
				}
			})
